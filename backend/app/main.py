...
def format_state_event() -> Event:
    return Event(
        EventType.STATE_CHANGED,
        {
            ...
            "context": state_machine.context.to_dict(),
        },
    )
...
async def process_payment_flow() -> None:
    try:
        ...
        if status != "authorized":
            status_payload = await payment_service.wait_for_authorization(transaction_id, timeout_seconds=config.simulator.payment_timeout_seconds if config.use_simulator else 30)
